...
export default function PostComposer({ profile, initials, colorClass, firstName }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)] p-3.5">
      <button
        type="button"
      ...
      >
        ...
        <span className="flex-1 rounded-full border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-400 transition-all duration-200 group-hover:bg-white group-hover:border-gray-300">
      ...
      <div className="mt-3 flex items-center gap-1.5 border-t border-gray-100 pt-3">
        {actions.map(({ key, label, icon: Icon, color }) => (
          <button
            ...
            type="button"
            onClick={() => navigate('/create')}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold text-gray-500 hover:bg-gray-50 transition-all duration-200"
          >
