  ...
  return (
    <Wrapper
      ...
      className={`w-full flex items-center gap-3 px-4 py-3.5 text-left ${
        onClick ? 'hover:bg-gray-50 dark:hover:bg-white/5 transition-all duration-300' : ''
      }`}
    >
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
          isDanger ? 'bg-red-50 text-red-600 dark:bg-red-500/15 dark:text-red-400' : 'bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400'
        }`}
      >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium ${isDanger ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-50'}`}>{label}</p>
        {description && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{description}</p>}
      </div>

      {rightElement || (onClick && <ChevronRight className="w-4 h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />)}
    </Wrapper>
  )
}